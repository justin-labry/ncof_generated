...
from nnef.apis.individual_subscription_document_api_base import BaseIndividualSubscriptionDocumentApi
import nnef.impl
...
async def delete_individual_subcription(
    subscriptionId: Annotated[StrictStr, Field(description="Event Subscription ID")] = Path(..., description="Event Subscription ID"),
    ...
    return await BaseIndividualSubscriptionDocumentApi.subclasses[0]().delete_individual_subcription(subscriptionId)


@router.get(
    "/subscriptions/{subscriptionId}",
    responses={
        200: {"model": NefEventExposureSubsc, "description": "OK. Resource representation is returned"},
        307: {"model": RedirectResponse, "description": "Temporary Redirect"},
        308: {"model": RedirectResponse, "description": "Permanent Redirect"},
        400: {"model": ProblemDetails, "description": "Bad request"},
        401: {"model": ProblemDetails, "description": "Unauthorized"},
...
async def get_individual_subcription(
    subscriptionId: Annotated[StrictStr, Field(description="Event Subscription ID")] = Path(..., description="Event Subscription ID"),
    supp_feat: Annotated[Optional[Annotated[str, Field(strict=True)]], Field(description="Features supported by the NF service consumer")] = Query(None, description="Features supported by the NF service consumer", alias="supp-feat", regex=r"/^[A-Fa-f0-9]*$/"),
    token_oAuth2ClientCredentials: TokenModel = Security(
        get_token_oAuth2ClientCredentials, scopes=["nnef-eventexposure"]
    ),
    ...
    if not BaseIndividualSubscriptionDocumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIndividualSubscriptionDocumentApi.subclasses[0]().get_individual_subcription(subscriptionId, supp_feat)


@router.put(
    "/subscriptions/{subscriptionId}",
    responses={
        200: {"model": NefEventExposureSubsc, "description": "OK. Resource was succesfully modified and representation is returned"},
...
async def replace_individual_subcription(
    ...
    if not BaseIndividualSubscriptionDocumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIndividualSubscriptionDocumentApi.subclasses[0]().replace_individual_subcription(subscriptionId, nef_event_exposure_subsc)
