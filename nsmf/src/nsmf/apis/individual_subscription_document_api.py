...
from nsmf.apis.individual_subscription_document_api_base import BaseIndividualSubscriptionDocumentApi
import nsmf.impl
...
@router.delete(
    "/subscriptions/{subId}",
    responses={
        200: {"model": EventNotification, "description": "OK. Resource was successfully deleted and representation is returned"},
...
async def delete_individual_subcription(
    subId: Annotated[StrictStr, Field(description="Event Subscription ID")] = Path(..., description="Event Subscription ID"),
    ...
    return await BaseIndividualSubscriptionDocumentApi.subclasses[0]().delete_individual_subcription(subId)


@router.get(
    "/subscriptions/{subId}",
    responses={
        200: {"model": NsmfEventExposure, "description": "OK. Resource representation is returned"},
        307: {"model": RedirectResponse, "description": "Temporary Redirect"},
        308: {"model": RedirectResponse, "description": "Permanent Redirect"},
        400: {"model": ProblemDetails, "description": "Bad request"},
        401: {"model": ProblemDetails, "description": "Unauthorized"},
...
async def get_individual_subcription(
    ...
    if not BaseIndividualSubscriptionDocumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIndividualSubscriptionDocumentApi.subclasses[0]().get_individual_subcription(subId)


@router.put(
    "/subscriptions/{subId}",
    responses={
        200: {"model": NsmfEventExposure, "description": "OK. Resource was successfully modified and representation is returned"},
...
async def replace_individual_subcription(
    ...
    if not BaseIndividualSubscriptionDocumentApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseIndividualSubscriptionDocumentApi.subclasses[0]().replace_individual_subcription(subId, nsmf_event_exposure)
