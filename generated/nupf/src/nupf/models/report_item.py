...
class ReportItem(BaseModel):
    ...
    """ # noqa: E501
    path: StrictStr = Field(description="Contains a JSON pointer value (as defined in IETF RFC 6901) that references a  location of a resource to which the modification is subject. ")
    reason: Optional[StrictStr] = Field(default=None, description="A human-readable reason providing details on the reported modification failure.  The reason string should identify the operation that failed using the operation's  array index to assist in correlation of the invalid parameter with the failed  operation, e.g. \"Replacement value invalid for attribute (failed operation index= 4)\". ")
    __properties: ClassVar[List[str]] = ["path", "reason"]
