...
Decision rule (single threshold, stateless):

    metric_mbps = sum of DL throughput from 12p_c
    if metric_mbps >= TH:    gNB2 -> ACTIVE       (WLAN saturated; cannot absorb offload)
    else:                    gNB2 -> DEEP_SLEEP   (WLAN has spare; offload feasible)

No hysteresis: the decision cadence (60 s subscription repPeriod) and
the input being already a 60 s mean (``dlAverageThroughput``) already
suppress high-frequency oscillation. Add an external minimum-dwell
guard at the engine layer (e.g. "no state change within 5 minutes")
only if real measurements show flap near the threshold.
...
QoS mapping per output policy set (driven by the new gNB2 state):
    ...
    | WLAN    | n3IwfId         | gbr=1000, mbr=1000| gbr=500,  mbr=500  |
    | NR      | gNB1 (000001)   | (untouched)       | (untouched)        |

When gNB2 wakes (ACTIVE), NR absorbs only ~50% of the Wi-Fi load; the
other half stays on Wi-Fi. Setting WLAN to 0 (NR 100% absorption) is
intentionally avoided as unrealistic. Total system capacity therefore
moves 1000 -> 1500 Mbps between DEEP_SLEEP and ACTIVE.

Run::

    # One-shot from PoC sample JSONs:
    python tools/decide_wlan_gnb2_rule.py

    # Override threshold:
    python tools/decide_wlan_gnb2_rule.py --th 800
...
TH_MBPS = 500.0  # >= : ACTIVE (WLAN saturated, can't offload), else: DEEP_SLEEP

GNB2_VALUE = "000002"  # which gNB the rule controls
GNB1_VALUE = "000001"  # primary gNB; never affected by this rule

GBR_HIGH_MBPS = (
    1000.0  # "fully on" gbr/mbr (3GPP-on-gNB2 when ACTIVE; WLAN when DEEP_SLEEP)
)
GBR_PARTIAL_MBPS = (
    500.0  # WLAN partial gbr/mbr when ACTIVE (NR takes ~50%, Wi-Fi keeps half)
)
GBR_OFF_MBPS = (
    0.0  # "fully off" gbr/mbr (3GPP-on-gNB2 when DEEP_SLEEP; cell physically off)
)
...
CELL_POWER_PRESETS: dict[str, dict[str, Any]] = {
    "DEEP_SLEEP": {
        "_cellTxPower": 20,
        "_ssPbchBlockPower": 15,
        "_siBlockPower": 10,
        "_pdschBlockPower": 18,
        "_pdcchBlockPower": 12,
        "_csiRsPowerOffset": 3,
        "_qRxLevMin": -120,
        "_cellIndividualOffset": 0,
    },
    "ACTIVE": {
        "_cellTxPower": 30,
        "_ssPbchBlockPower": 25,
        "_siBlockPower": 20,
        "_pdschBlockPower": 28,
        "_pdcchBlockPower": 22,
        "_csiRsPowerOffset": 3,
...
def decide_gnb2_state(
    metric_mbps: float,
    th_mbps: float = TH_MBPS,
) -> str:
    """Single-threshold rule (stateless).

    metric >= th_mbps  -> ACTIVE       (WLAN saturated, gNB2 must stay awake)
    metric <  th_mbps  -> DEEP_SLEEP   (WLAN has headroom, gNB2 can sleep)

    No hysteresis: 60 s decision cadence + dlAverageThroughput already being
    a 60 s mean naturally suppress high-frequency oscillation. If threshold
    flap is observed in practice, prefer adding a minimum-dwell guard at the
    engine layer (e.g. "no state change within 5 minutes") over reintroducing
    a hysteresis band -- it is easier to reason about and composes with the
    RL stage's action masking.
    """
    return "ACTIVE" if metric_mbps >= th_mbps else "DEEP_SLEEP"
                    ...
                    "_cellPowerCtrlOptInfos": [
                        {
                            "_cellPowerCtrlInfos": [
                                {
                                    "_tsStart": start,
                                    "_tsDuration": int(SUB_DURATION.total_seconds()),
                                    "_ratFreq": {
                                        "allFreq": True,
                                        "allRat": False,
                                        "ratType": "NR",
...
def apply_qos_policy(template: list[dict], state: str) -> list[dict]:
    ...
    Rules:
        - WLAN set:         gbr/mbr = HIGH    if DEEP_SLEEP   (Wi-Fi absorbs all)
                            gbr/mbr = PARTIAL if ACTIVE       (NR takes ~50%, Wi-Fi keeps half)
        - NR + gNB2 set:    gbr/mbr = OFF     if DEEP_SLEEP   (cell physically off)
...
def _flip_one_set(policy_set: dict, state: str) -> None:
    if _is_wlan_set(policy_set):
        # DEEP_SLEEP: WLAN absorbs everything (HIGH).
        # ACTIVE:    WLAN keeps ~50% (PARTIAL); NR takes the other half.
        #            Setting WLAN to OFF would mean NR absorbs 100%, which is
        #            unrealistic -- avoided intentionally.
        gbr = GBR_HIGH_MBPS if state == "DEEP_SLEEP" else GBR_PARTIAL_MBPS
    elif _is_on_gnb(policy_set, GNB2_VALUE):
        # DEEP_SLEEP: gNB2 physically off (OFF).
        # ACTIVE:    gNB2 at full capacity (HIGH).
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        default=TH_MBPS,
        help=f"WLAN DL throughput threshold in Mbps (default: {TH_MBPS})",
    )
    p.add_argument(
        "--decision-time",
        default=None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    new_state = decide_gnb2_state(metric, args.th)
    ...
    print(
        f"[rule] metric={metric:.1f} Mbps  th={args.th} Mbps  -> {new_state}",
        file=sys.stderr,
    )
